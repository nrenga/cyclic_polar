...
k = rate*N;
q = 17;
f = gf_fft_rate_design_bec(n,k,q);  % Design is same with and without twiddle
% e = 0.5;
...
for e = 0.1:0.1:1
    ...
    for i = 1:M
        ...
        % Decode
        [uhat,xhatprob] = gf_fft_decode(y,f,q,[],[],N);
%         xhat = zeros(1,N);
%         for j = 1:N
%             if (recv(j) == 1/q)
...
t = strcat('GF-FFT Code Performance; Design Rate = ',num2str(rate));

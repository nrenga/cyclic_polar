function [ y ] = gf_ifft(u,p,m,omega,N)
...
if (mod(p^m-1,N) == 0)
    if (N == len)
        ...
        for i = 1:(p^m-2)   % Find the smallest primitive Nth root of unity
            ...
            if (any(a==0) && find(a==0,1,'first') == N)
                omega = i;
                omega = gfdiv(0,omega,field);  % Find and use omega^-1
    ...
    if (omega_valid == 1)
        if (isprime(len))
            ...
            y = (-1)*ones(no_of_blocks,lengths(1));
            for i = 1:size(z,1)
                y(i,:) = (gf_ifft(z(i,:),p,m,omega,N))';
            end
            y = (reshape(y,1,[]))';
        end
    else

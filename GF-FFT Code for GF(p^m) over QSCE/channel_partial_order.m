function [partial_order, violations, permutations, inds_i, I, I_coords, O, O_coords, E, Ecomp, bases] = channel_partial_order(lengths,e)
            ...
            if (isempty(find(accounted_inds == O(j),1)))
                ...
                if (perm_index < j)
                    ...
                    if (all( ismember(O(1:perm_index-1),inds_i{no_of_perm}(O(1:j-1)+1)) ))
                        ...
                        partial_order{order_index,1} = [O(j) O_permuted_inds(j)];
...
fprintf('\nTotal # of pair-orderings = %d and there were %d violations.\n\n',order_index,v_ind);

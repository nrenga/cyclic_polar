    ...
    for j = 1:len
        ...
        for k = 1:no_of_blocks
            if (nnz(y(:,k) == 1/p^m) <= j) % #outputs erased <= #inputs known
                inp = x(1:j,k);
                [x(:,k), op_stage] = bm_forney_fft(y(:,k),inp,p,m,mod(omega*(N/length(y(:,k))),p^m-1),field,verbose);
            end
        end
...
return
    ...
    if (no_of_syndromes > 0)
        ...
        if (~all(S == -Inf))    % Proceed if received word isn't a codeword
            ...
            if (no_of_syndromes >= 2*no_of_errors + no_of_erasures)
        ...
        inp_est(inp ~= 1/q) = inp(inp ~= 1/q);
    else
        inp_est = gf_fft(r_est,p,m,[],length(r_est));   % No errors/erasures
    end
    if (verbose >= 1)
